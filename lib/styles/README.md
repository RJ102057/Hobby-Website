...
- **Color** — `@theme` in `css/tailwind.css` is the single source of truth for
  the raw brand palette (`--color-ink`, `--color-ivory`, `--color-stone`,
  `--color-accent`, `--color-accent-deep`) plus the semantic
  `--color-primary` / `--color-secondary` / `--color-contrast` roles that map
  onto it (ivory/ink/accent). There is a single fixed palette — no runtime
  theme switching. Tailwind v4 compiles `@theme` into `:root` custom
  properties, so there is no separate `:root` copy. Reference the semantic
  tokens (or the raw brand tokens directly) — never hard-code a hex in a
  component.
