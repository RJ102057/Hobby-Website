...
Named shortcut aliases: `@/hooks/*`, `@/styles/*`, `@/integrations/*`, `@/utils/*`, `@/config`, `@/dev`, `@/dev/*`. Everything else — including `@/components/*` and `@/lib/*` — resolves through the `@/*` root catch-all (`@/*` maps to the repo root in `tsconfig.json`, so e.g. `@/components/ui/link` resolves to `components/ui/link`).
...
- Honor reduced-motion: the global neutralizer in `global.css` zeroes CSS animation; JS gates via `usePreferredReducedMotion`.
...
- Global state: Zustand (dedicated store files, e.g. `lib/dev/orchestra.ts`)
...
### 6. useRef for Object Instantiation
...
Good for: tabs, carousels, accordions. The component pre-renders without performance impact and automatically cleans up effects when hidden.
...
- Sanctioned exceptions, each for a non-CSS parser or spec that cannot use oklch: `.storybook/manager.ts` (storybook/theming → polished). Anything new that must stay non-oklch needs a comment stating which parser or spec forces it.
